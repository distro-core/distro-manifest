...
[[ ! grep -s "dockremap" /etc/subuid ]] && sudo ${sudo_args} -s bash -c "echo dockremap:4294936224:65536 >>/tmp/subuid"
[[ ! grep -s "dockremap" /etc/subgid ]] && sudo ${sudo_args} -s bash -c "echo dockremap:4294936224:65536 >>/tmp/subgid"

[[ ! grep -s "runner" /etc/subuid ]] && sudo ${sudo_args} -s bash -c "echo runner:200000:65536 >>/tmp/subuid"
[[ ! grep -s "runner" /etc/subgid ]] && sudo ${sudo_args} -s bash -c "echo runner:200000:65536 >>/tmp/subgid"
...
sudo ${sudo_args} -s systemctl stop docker.socket
