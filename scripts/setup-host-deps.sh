...
almalinux* ) 
    ...
    sudo ${sudo_args} -s dnf install -y -q gawk make wget tar bzip2 gzip python3 unzip perl patch \
                            ...
                            rpcgen zstd lz4 cpio glibc-langpack-en libacl git-lfs hostname repo s3cmd \
                            python3-websockets jq cifs-utils
...
debian* | ubuntu* )
    ...
    sudo ${sudo_args} -s apt-get install -y -q gawk wget git diffstat unzip texinfo gcc build-essential \
                            ...
                            file locales libacl1 git-lfs make libxml2-utils repo s3cmd python3-websockets \
                            time jq cifs-utils
...
: install repo tool
sudo ${sudo_args} -s install -d /usr/local/bin
sudo ${sudo_args} -s curl --silent https://storage.googleapis.com/git-repo-downloads/repo >/usr/local/bin/repo
sudo ${sudo_args} -s chmod a+rx /usr/local/bin/repo
